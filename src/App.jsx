...
const APP_VERSION = "Castan Realtime v2.3.0-fechamento-contratos";
...
export default function App(){
  ...
  const enviadasFechamento = visitas.filter(v=>
    v.status==="avancou_fechamento" && v.checklist_ok
  );
  ...
  async function saveVisit(){
    ...
    if(f.status==="avancou_fechamento" && !f.checklist_ok){
      const confirmarChecklist = window.confirm("CHECK LISTS OK?");
...
const visitasCanceladasBase=visitas
  ...
  const minhaAgendaVisits=visitas
    .filter(v=>{
      if(isFechamento){
        return ["concluida","avancou_fechamento","pos_ok"].includes(v.status);
      }

      if(isContratos){
        return v.status==="avancou_fechamento" && v.checklist_ok;
      }
        ...
        <main className="content">
          ...
          {view==="fechamento"&&
            <Card title="Visitas enviadas para fechamento">
              {enviadasFechamento.map(v=>
                <VisitCard key={v.id} v={v} getUser={getUser} colorForUser={colorForUser} onClick={()=>openVisit(v)}/>
              )}
              {!enviadasFechamento.length&&<Empty text="Nenhuma visita enviada para fechamento."/>}
            </Card>
          }
  ...
  return <div className="overlay">
    <div className="modal">
      ...
      <div className="form">
        ...
        {f.status==="avancou_fechamento"&&
          <label className="checkline">
            <input type="checkbox" checked={Boolean(f.checklist_ok)} onChange={e=>setF({...f,checklist_ok:e.target.checked})}/>
            <span>Check list / enviar para contrato</span>
          </label>
        }
