...
const APP_VERSION = "Castan Realtime v2.1.5-canceladas-logo-fix";
...
export default function App(){
  ...
  const minhaAgendaVisits=visitas
    .filter(v=>["agendada","confirmada"].includes(v.status))
    .filter(v=>{
        ...
        <main className="content">
          ...
          {view==="inicio"&&<>
            <section className="metrics">
              ...
              <Metric title="Canceladas" value={visitas.filter(v=>v.status==="cancelada").length} onClick={()=>setView("canceladas")}/>
            ...
            <section className="home-layout">
              ...
              <aside className="home-sidebar">
                ...
                <Card title="Ranking Captadores">
                  <SimpleBars rows={rankingCaptadores}/>
                </Card>
...
function CanceladasView({
  visitas,preUsers,mostradores,getUser,colorForUser,openVisit,
  busca,setBusca,pre,setPre,mostrador,setMostrador,motivo,setMotivo,start,setStart,end,setEnd
}){
  const base = visitas
    .filter(v=>v.status==="cancelada")
    .sort((a,b)=>(String(b.data_visita||"")+String(b.horario_visita||"")).localeCompare(String(a.data_visita||"")+String(a.horario_visita||"")));
  ...
  const filtradas = base.filter(v=>{
    ...
    const byPre=pre==="all" || v.pre_atendimento_id===pre;
    const byMostrador=mostrador==="all" || v.mostrador_id===mostrador;
    const byMotivo=motivo==="all" || v.motivo_cancelamento===motivo;
    const byStart=!start || v.data_visita>=start;
    const byEnd=!end || v.data_visita<=end;

    return bySearch && byPre && byMostrador && byMotivo && byStart && byEnd;
  });

  return <Card title="Visitas canceladas">
    <div className="canceladas-header">
      <div>
        <h3>Total canceladas: {base.length}</h3>
    ...
    <div className="canceladas-filtros">
      ...
      <select value={mostrador} onChange={e=>setMostrador(e.target.value)}>
    ...
    {!filtradas.length&&<Empty text="Nenhuma visita cancelada encontrada."/>}
  </Card>;
}
...
function VisitCard({v,getUser,colorForUser,onClick}){
  return <div className="visitcard" style={{borderLeftColor:colorForUser(v.mostrador_id)}} onClick={onClick}>
    <div>
      ...
      {v.status==="cancelada"&&v.motivo_cancelamento&&<p>Motivo: {v.motivo_cancelamento}</p>}
  ...
  return <div className="overlay">
    <div className="modal">
      ...
      <div className="form">
        ...
        <Select label="Status *" value={f.status} disabled={limited} onChange={v=>setF({...f,status:v,motivo_cancelamento:v==="cancelada"?f.motivo_cancelamento:""})} options={statusOptions}/>

        {f.status==="cancelada"&&<Select label="Motivo do cancelamento *" value={f.motivo_cancelamento||""} disabled={limited} onChange={v=>setF({...f,motivo_cancelamento:v})} options={MOTIVOS_CANCELAMENTO.map(m=>[m,m])}/>}
