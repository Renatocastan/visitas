...
const APP_VERSION = "Castan Realtime v3.1.5-calendario-interativo";
...
function Calendar({year,month,visitas,bloqueios=[],colorForUser,getUser,onNew,onEdit}){
  ...
  return <div className="month-scheduler month-scheduler-list" onMouseLeave={()=>setTooltip(null)}>
    ...
    <div className="month-scheduler-grid">
      ...
      {Array.from({length:days}).map((_,i)=>{
        ...
        return <div className="month-day month-day-list" key={day}>
          <div className="month-day-number">{day}</div>
          <div className="month-day-agenda">
            {hours.map(h=>{
              const hora=`${String(h).padStart(2,"0")}:00`;
              const items=eventos.filter(ev=>{
                const inicio=String(ev.tipo==="bloqueio"?ev.item.horario_inicio:ev.item.horario_visita).slice(0,2);
                return Number(inicio)===h;
              });

              return <div className="month-hour-row" key={h} onClick={()=>onNew(date,hora)}>
                <span className="month-hour-label">{String(h).padStart(2,"0")}</span>
                <div className="month-hour-items">
                  {items.map(ev=>{
                    ...
                    const v=ev.item;
                    return <div key={ev.id} className={`month-list-event ${v.atualizar_fotos?"foto-destaque":""}`} style={{borderLeftColor:colorForUser(v)}} onMouseEnter={e=>setTip(e,visitTooltip(v,getUser))} onMouseMove={e=>setTip(e,visitTooltip(v,getUser))} onMouseLeave={()=>setTooltip(null)} onClick={e=>{e.stopPropagation();onEdit(v)}}>
                      <strong>{String(v.horario_visita).slice(0,5)}-{String(v.horario_fim_visita||v.horario_visita).slice(0,5)} • {v.status==="reserva"?"RESERVA":v.codigo_imovel}</strong>
...
function WeeklyCalendar({weekStart,visitas,bloqueios=[],colorForUser,getUser,onNew,onEdit}){
  ...
  return <div className="week-scheduler week-scheduler-list" onMouseLeave={()=>setTooltip(null)}>
    ...
    <div className="week-list-body">
      <div className="time-col">
        {hours.map(h=><div key={h} className="week-list-time-label">{String(h).padStart(2,"0")}:00</div>)}
      </div>
      ...
      {days.map(d=>{
        ...
        return <div className="week-list-day" key={date}>
          {hours.map(h=>{
            const hora=`${String(h).padStart(2,"0")}:00`;
            const items=eventos.filter(ev=>{
              const inicio=String(ev.tipo==="bloqueio"?ev.item.horario_inicio:ev.item.horario_visita).slice(0,2);
            ...
            return <div className="week-list-hour-row" key={h} onClick={()=>onNew(date,hora)}>
              {items.map(ev=>{
                if(ev.tipo==="bloqueio"){
                  const b=ev.item;
                ...
                const v=ev.item;
                return <div key={ev.id} className={`week-list-event ${v.atualizar_fotos?"foto-destaque-card":""}`} style={{borderLeftColor:colorForUser(v)}} onMouseEnter={e=>setTip(e,visitTooltip(v,getUser))} onMouseMove={e=>setTip(e,visitTooltip(v,getUser))} onMouseLeave={()=>setTooltip(null)} onClick={e=>{e.stopPropagation();onEdit(v)}}>
                  <strong>{String(v.horario_visita).slice(0,5)}-{String(v.horario_fim_visita||v.horario_visita).slice(0,5)} • {v.status==="reserva"?"RESERVA":v.codigo_imovel}</strong>
                  <span>{v.status==="reserva"?(getUser?.(v.mostrador_id)?.nome||"Reserva"):v.cliente_nome}</span>
                </div>;
              })}
