...
const APP_VERSION = "Castan Realtime v3.0.8-cores-reserva";
...
export default function App(){
  ...
  async function saveVisit(){
    ...
    if(isReservaAgenda){
      if(!user?.id || !f.mostrador_id || !f.data_visita || !f.horario_visita || !f.status){
    ...
    const payload={
      ...
      pre_atendimento_id:user?.id||f.pre_atendimento_id,
    ...
    {days.map(d=>{
      ...
      return <div className={`weekly-day ${isToday?"today":""}`} key={date} onClick={()=>onNew(date)}>
        ...
        {arr.length?arr.map(v=>
          <div className={`weekly-visit ${v.atualizar_fotos?"foto-destaque-card":""}`} data-tooltip={visitTooltip(v,getUser)} key={v.id} style={{borderLeftColor:colorForUser(v)}} onClick={e=>{e.stopPropagation();onEdit(v)}}>
            <strong>{String(v.horario_visita).slice(0,5)} • {v.status==="reserva"?"RESERVA":v.codigo_imovel}</strong>
            <span>{v.status==="reserva"?(getUser?.(v.mostrador_id)?.nome||"Reserva de agenda"):v.cliente_nome}</span>
          </div>
