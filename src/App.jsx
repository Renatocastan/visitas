...
const APP_VERSION = "Castan Realtime v2.3.2-fechamento-cancelamento";
...
function VisitModal({f,setF,onClose,onSave,onDelete,isAdmin,isGestor,isMostrador,isFechamento,isContratos,canDeleteVisits,canStatusAvancouFechamento,canStatusPosOk,canStatusContrato,preUsers,mostradores,editing,acoes=[],fotos=[],getUser,onCaptureLocation,onUploadFiles}){
  ...
  return <div className="overlay">
    ...
    motivo_cancelamento_outros:""})} options={statusOptions}/>
        ...
        {f.status==="cancelada"&&<Select label="Motivo do cancelamento *" value={f.motivo_cancelamento||""} disabled={limited} onChange={v=>setF({...f,motivo_cancelamento:v,motivo_cancelamento_outros:v==="outros"?f.motivo_cancelamento_outros:""})} options={MOTIVOS_CANCELAMENTO.map(m=>[m,m])}/>

          {f.motivo_cancelamento==="outros"&&
            <Field label="Descreva o motivo do cancelamento *" value={f.motivo_cancelamento_outros||""} disabled={limited} onChange={v=>setF({...f,motivo_cancelamento_outros:v})}/>
          }}
