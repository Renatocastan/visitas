...
const APP_VERSION = "Castan Realtime v3.0.9-grade-horarios-bloqueios";
...
function Calendar({year,month,visitas,colorForUser,getUser,onNew,onEdit}){
  ...
  return <>
    ...
    <div className="calendar calendar-tooltip-host" onMouseLeave={()=>setTooltip(null)}>
      ...
      {Array.from({length:days}).map((_,i)=>{
        ...
        return <div className="day" key={day} onClick={()=>onNew(date)}>
          ...
          {arr.map(v=>
            <div
              className={`chip ${v.atualizar_fotos?"foto-destaque":""}`}
              key={v.id}
              style={{background:colorForUser(v)}}
            ...
            >
              ...
              <span>{v.status==="reserva"?(getUser?.(v.mostrador_id)?.nome||"Reserva de agenda"):(v.cliente_nome||"")}</span>
            </div>
          )}
        </div>;
      })}
...
function WeeklyCalendar({weekStart,visitas,colorForUser,getUser,onNew,onEdit}){
  ...
  return <div className="week-scheduler">
        ...
        return <div className="week-scheduler-day" key={date} onClick={()=>onNew(date)}>
          ...
          {arr.map(v=>{
            ...
            return <div
              ...
              data-tooltip={visitTooltip(v,getUser)}
