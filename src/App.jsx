...
const APP_VERSION = "Castan Realtime v3.1.1-cards-inicio";
        ...
        <main className="content">
          ...
          {view==="calendario"&&<>
            ...
            <Card>
              ...
              {calendarMode==="month"
                ? <Calendar year={year} month={month} visitas={visibleVisits.filter(v=>v.status!=="cancelada")} colorForUser={colorForVisit} getUser={getUser} onNew={startNewVisit} onEdit={openVisit}/>
                : calendarMode==="week"
                  ? <WeeklyCalendar weekStart={weekStart} visitas={visibleVisits.filter(v=>v.status!=="cancelada")} colorForUser={colorForVisit} getUser={getUser} onNew={startNewVisit} onEdit={openVisit}/>
                  : <DailyCalendar currentDay={currentDay} visitas={visibleVisits.filter(v=>v.status!=="cancelada")} colorForUser={colorForVisit} getUser={getUser} onNew={startNewVisit} onEdit={openVisit}/>
              }
            </Card>
          </>}
...
function Calendar({year,month,visitas,colorForUser,getUser,onNew,onEdit}){
  ...
  function minutesFromStart(hora){
    ...
    return Math.max(0,((h||8)-8)*60+(m||0));
  }

  return <div className="month-scheduler" onMouseLeave={()=>setTooltip(null)}>
    <div className="month-scheduler-weekdays">
        ...
        return <div className="month-day" key={day} onClick={()=>onNew(date)}>
          ...
          <div className="month-day-hours">
            ...
            {arr.map(v=>{
              ...
              return <div
                key={v.id}
                className={`month-event ${v.atualizar_fotos?"foto-destaque":""}`}
                style={{top, height, background:colorForUser(v)}}
                onMouseEnter={e=>showTooltip(e,v)}
                onMouseMove={e=>showTooltip(e,v)}
...
function WeeklyCalendar({weekStart,visitas,colorForUser,getUser,onNew,onEdit}){
  ...
  const gridVisits=visitas
    .filter(v=>days.some(d=>dateStr(d)===v.data_visita))
  ...
  return <div className="week-scheduler" onMouseLeave={()=>setTooltip(null)}>
    ...
    <div className="week-scheduler-body">
      <div className="time-col">
        {hours.map(h=><div key={h} className="time-slot-label">{String(h).padStart(2,"0")}:00</div>)}
      </div>

      {days.map(d=>{
        const date=dateStr(d);
        const arr=gridVisits.filter(v=>v.data_visita===date);
        return <div className="week-scheduler-day" key={date} onClick={()=>onNew(date)}>
          ...
          {arr.map(v=>{
            ...
            return <div
              key={v.id}
              className={`week-event ${v.atualizar_fotos?"foto-destaque-card":""}`}
              style={{top, height, borderLeftColor:colorForUser(v)}}
              onMouseEnter={e=>showTooltip(e,v)}
              onMouseMove={e=>showTooltip(e,v)}
              onMouseLeave={()=>setTooltip(null)}
              onClick={e=>{e.stopPropagation();onEdit(v)}}
            >
              <strong>{String(v.horario_visita).slice(0,5)}-{String(fim).slice(0,5)} • {v.status==="reserva"?"RESERVA":v.codigo_imovel}</strong>
              <span>{v.status==="reserva"?(getUser?.(v.mostrador_id)?.nome||"Reserva"):v.cliente_nome}</span>
            </div>
          })}
        </div>;
  ...
  return <div className="daily-calendar">
    <div className="daily-day" onClick={()=>onNew(currentDay)}>
      ...
      {arr.length?arr.map(v=>
        <div className={`weekly-visit daily-visit ${v.atualizar_fotos?"foto-destaque-card":""}`} data-tooltip={visitTooltip(v,getUser)} key={v.id} style={{borderLeftColor:colorForUser(v)}} onClick={e=>{e.stopPropagation();onEdit(v)}}>
          <strong>{String(v.horario_visita).slice(0,5)}-{String(v.horario_fim_visita||v.horario_visita).slice(0,5)} • {v.codigo_imovel}</strong>
