...
const APP_VERSION = "Castan Realtime v2.5.1-bloqueio-agenda-fix";
      ...
      <>
        <nav className="tabs">
          {[
            ...
            ...(isMostrador?[["bloquear_agenda",CalendarDays,"Bloquear agenda"]]:[]),
        ...
        <main className="content">
          ...
          {view==="bloquear_agenda"&&isMostrador&&
            <Card title="Bloquear minha agenda">
              <p className="hint">Use este botão para bloquear horários em que você não poderá fazer visitas.</p>
              ...
              <div className="bloqueio-lista">
                {agendaBloqueios
                  .filter(b=>b.ativo!==false&&b.usuario_id===user?.id)
                  .map(b=>
                    <div key={b.id} className="bloqueio-card">
                      <strong>{String(b.data_bloqueio||"").split("-").reverse().join("/")} • {String(b.horario_inicio||"").slice(0,5)} às {String(b.horario_fim||"").slice(0,5)}</strong>
                      <p>{b.justificativa}</p>
                      <button className="btn ghost" onClick={()=>removerBloqueioAgenda(b.id)}>Remover bloqueio</button>
                    </div>
                  )
                }
                {!agendaBloqueios.filter(b=>b.ativo!==false&&b.usuario_id===user?.id).length&&<Empty text="Nenhum bloqueio ativo."/>}
              </div>
            </Card>
          }
