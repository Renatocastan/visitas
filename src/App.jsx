...
const APP_VERSION = "Castan Realtime v3.1.6-grade-30min";
...
function Calendar({year,month,visitas,bloqueios=[],colorForUser,getUser,onNew,onEdit}){
  ...
  const slots=Array.from({length:26}).map((_,i)=>{
  ...
  return <div className="week-scheduler week-scheduler-list" onMouseLeave={()=>setTooltip(null)}>
    ...
    <div className="week-list-body">
      <div className="time-col">
        {slots.map(slot=><div key={slot} className={`week-list-time-label ${slot.endsWith(":30")?"half-hour-label":""}`}>{slot}</div>)}
      </div>
      ...
      {days.map(d=>{
        ...
        return <div className="week-list-day" key={date}>
          {slots.map(slot=>{
            ...
            return <div className={`week-list-hour-row halfhour-row ${slot.endsWith(":30")?"half-hour-row":""}`} key={slot} onClick={()=>onNew(date,slot)}>
              {items.map(ev=>{
                if(ev.tipo==="bloqueio"){
                  const b=ev.item;
