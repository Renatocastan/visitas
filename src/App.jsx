...
const APP_VERSION = "Castan Realtime v1.6.0-ajustes-operacionais";
...
function emptyVisit(user, preUsers, mostradores){
  return {
    ...
    cliente_nome:"",
    cliente_contato:"",
    pre_atendimento_id:user?.tipo==="pre_atendimento"?user.id:(preUsers[0]?.id||""),
...
export default function App(){
  ...
  const canCreateVisit=isAdmin||isGestor||isPre;
  const canManageUsers=isAdmin||isGestor;

  const loadAll=useCallback(async()=>{
    try{
      const [u,v,n,a,ft]=await Promise.all([
      ...
      if(!n.error){
        ...
        if(currentUserId){
          ...
          if(myUnread.length){
            setToast(myUnread);
            myUnread.forEach(item=>seenNotificationsRef.current.add(item.id));
  ...
  function canEditVisit(v){
    if(isAdmin||isGestor||isPre||isFechamento||isContratos)return true;
    if(isMostrador)return v.mostrador_id===user?.id;
    return false;
  }
  ...
  async function notifyMany(ids,titulo,mensagem){
    ...
    const filtered = unique.filter(id=>{
      ...
      return u && u.tipo !== "admin" && u.ativo !== false;
    });
  ...
  },[visitas,filterPre,filterMostrador,filterStatus,searchTerm,usuarios,isContratos]);
  ...
  const reportVisits=visitas.filter(v=>v.data_visita===reportDate);
  ...
  const statusResumo=STATUS.map(([id,label])=>({
    id,
    label,
    total:visitas.filter(v=>v.status===id).length
  })).filter(x=>x.total>0);

  const rankingPre=preUsers.map(p=>({
    nome:p.nome,
    total:visitas.filter(v=>v.pre_atendimento_id===p.id).length
  ...
  function exportReport(){
    ...
    exportCsv(`castan-relatorio-${reportDate}.csv`,rows);
  }
  ...
  function openVisit(v){
    if(!canEditVisit(v))return alert("Você pode visualizar, mas não pode editar esta visita.");
    setModalVisit({...v,valor_proposta:v.valor_proposta||""});
  }
  ...
  return <div className="app" onTouchStart={e=>{touchStart.current=e.touches[0].clientY}} onTouchEnd={e=>{const end=e.changedTouches[0].clientY;if(window.scrollY===0&&end-touchStart.current>80)loadAll()}}>
    ...
    <header className="header">
      ...
      <div className="header-actions">
        ...
        {user&&<div className="notif"><Bell size={16}/> {unread}</div>}
    ...
    {!user ? (
      <main className="select-user">
        ...
        <p className="hint">Senha inicial padrão: 123456. O ADM/Gestor pode alterar a senha na aba Equipe.</p>
      </main>
    ) : (
      <>
        <nav className="tabs">
        ...
        <main className="content">
          ...
          {view==="inicio"&&<>
            <section className="metrics">
              ...
              <Metric title="Canceladas" value={todayVisits.filter(v=>v.status==="cancelada").length}/>
          ...
          {view==="calendario"&&<>
            ...
            <Card>
              ...
              {calendarMode==="month"
                ? <Calendar year={year} month={month} visitas={visibleVisits} colorForUser={colorForUser} onNew={startNewVisit} onEdit={openVisit}/>
                : calendarMode==="week"
                  ? <WeeklyCalendar weekStart={weekStart} visitas={visibleVisits} colorForUser={colorForUser} onNew={startNewVisit} onEdit={openVisit}/>
                  : <DailyCalendar currentDay={currentDay} visitas={visibleVisits} colorForUser={colorForUser} onNew={startNewVisit} onEdit={openVisit}/>
              }
            </Card>
          </>}
          ...
          {view==="relatorios"&&
            <Card title="Relatórios">
              ...
              <ActionTable acoes={acoes} visitas={visitas} getUser={getUser} reportDate={reportDate}/>
            </Card>
          }
...
function Metric({title,value}){return <div className="metric"><strong>{value}</strong><span>{title}</span></div>}
...
function Calendar({year,month,visitas,colorForUser,onNew,onEdit}){
    ...
    <div className="calendar">
      ...
      {Array.from({length:days}).map((_,i)=>{
        ...
        return <div className="day" key={day} onClick={()=>onNew(date)}>
          <b>{day}</b>
          {arr.slice(0,4).map(v=>
            <div className="chip" key={v.id} style={{background:colorForUser(v.mostrador_id)}} onClick={e=>{e.stopPropagation();onEdit(v)}}>
              {String(v.horario_visita).slice(0,5)} {v.codigo_imovel}
            </div>
          )}
          {arr.length>4&&<small>+{arr.length-4}</small>}
        </div>;
      })}
    </div>
  </>;
...
function WeeklyCalendar({weekStart,visitas,colorForUser,onNew,onEdit}){
    ...
    {days.map(d=>{
      ...
      return <div className={`weekly-day ${isToday?"today":""}`} key={date} onClick={()=>onNew(date)}>
        ...
        {arr.length?arr.map(v=>
          <div className="weekly-visit" key={v.id} style={{borderLeftColor:colorForUser(v.mostrador_id)}} onClick={e=>{e.stopPropagation();onEdit(v)}}>
...
function DailyCalendar({currentDay,visitas,colorForUser,onNew,onEdit}){
  const d = new Date(currentDay+"T00:00:00");
  const arr=visitas
  ...
  return <div className="daily-calendar">
    <div className="daily-day" onClick={()=>onNew(currentDay)}>
      ...
      {arr.length?arr.map(v=>
        <div className="weekly-visit daily-visit" key={v.id} style={{borderLeftColor:colorForUser(v.mostrador_id)}} onClick={e=>{e.stopPropagation();onEdit(v)}}>
          <strong>{String(v.horario_visita).slice(0,5)} • {v.codigo_imovel}</strong>
          <span>{v.cliente_nome}</span>
...
function ActionTable({acoes,visitas,getUser,reportDate}){
  const rows=acoes.filter(a=>visitas.find(v=>v.id===a.visita_id)?.data_visita===reportDate);
  return <div className="tablewrap">
    <table>
...
function VisitModal({f,setF,onClose,onSave,onDelete,isAdmin,isGestor,isMostrador,isFechamento,isContratos,preUsers,mostradores,editing,acoes=[],fotos=[],getUser,onCaptureLocation,onUploadFiles}){
  const limited=isMostrador&&!isAdmin&&!isGestor;
  const historico=(acoes||[]).filter(a=>a.visita_id===f.id);
  const visitFotos=(fotos||[]).filter(x=>x.visita_id===f.id);
  const statusOptions = isContratos
    ? STATUS.filter(([id])=>id==="contrato")
    : isFechamento
      ? STATUS.filter(([id])=>id!=="contrato")
      : STATUS.filter(([id])=>!["pos_ok","contrato"].includes(id));

  return <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
    <div className="modal">
      <div className="modalhead">
        <h2>{editing?"Editar visita":"Nova visita"}</h2>
      ...
      <div className="form">
        ...
        <Select label="Pré atendimento *" value={f.pre_atendimento_id} disabled={limited} onChange={v=>setF({...f,pre_atendimento_id:v})} options={preUsers.map(u=>[u.id,u.nome])}/>
        ...
        <Field label="Horário *" type="time" value={String(f.horario_visita||"").slice(0,5)} disabled={limited} onChange={v=>setF({...f,horario_visita:v})}/>
        <Select label="Status *" value={f.status} onChange={v=>setF({...f,status:v})} options={statusOptions}/>
        ...
        <label className="check">
          <input
            ...
            disabled={limited}
            onChange={e=>setF({...f,atualizar_fotos:e.target.checked})}
          /> ATUALIZAR FOTOS
        </label>

        {isFechamento&&<label className="check"><input type="checkbox" checked={!!f.checklist} onChange={e=>setF({...f,checklist:e.target.checked,status:e.target.checked?"avancou_fechamento":f.status})}/> Check list / enviar para contrato</label>}
        {isFechamento&&<Field label="Valor da proposta *" type="number" value={f.valor_proposta||""} onChange={v=>setF({...f,valor_proposta:v})}/>}
        {isFechamento&&<label className="check"><input type="checkbox" checked={!!f.contrato_fechado} onChange={e=>setF({...f,contrato_fechado:e.target.checked,status:e.target.checked?"contrato":f.status})}/> Virou contrato</label>}
      ...
      {editing&&<div style={{marginTop:16}}>
        ...
        <div className="notification">
          ...
          <button className="btn ghost" onClick={onCaptureLocation}><Navigation size={16}/> Capturar localização atual</button>
        </div>
      </div>}
      ...
      <div className="modalactions">
        {editing&&(isAdmin||isGestor)&&<button className="btn danger" onClick={()=>onDelete(f.id)}><Trash2 size={16}/> Excluir</button>}
        <button className="btn ghost" onClick={onClose}>Cancelar</button>
        <button className="btn primary" onClick={onSave}><Save size={16}/> Salvar</button>
      </div>
    </div>
  </div>;
}

function UserModal({u,setU,onClose,onSave}){
  return <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
    <div className="modal smallmodal">
      ...
      <Select label="Perfil *" value={u.tipo} onChange={v=>setU({...u,tipo:v})} options={Object.entries(ROLES).map(([k,v])=>[k,v])}/>
