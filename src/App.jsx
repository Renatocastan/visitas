...
const APP_VERSION = "Castan Realtime v1.5.0-equipe-posok-popup";
...
export default function App(){
  ...
  },[visitas,filterPre,filterMostrador,filterStatus,searchTerm,usuarios]);
  ...
  const minhaAgendaVisits=visitas
    .filter(v=>isMostrador?v.mostrador_id===user?.id:true)
    ...
    {!user ? (
      ...
      <>
        ...
        <main className="content">
          ...
          {view==="inicio"&&<>
            ...
            <section className="grid2">
              ...
              <Card title="Notificações">
                {notificacoes.filter(n=>n.usuario_id===user.id).slice(0,12).map(n=>
                ...
                {!notificacoes.filter(n=>n.usuario_id===user.id).length&&<Empty text="Nenhuma notificação."/>}
              </Card>
            </section>
          ...
          {view==="calendario"&&<>
            ...
            <Card>
              <div className="monthbar">
                ...
                <strong>{calendarMode==="month"?`${MONTHS[month]} ${year}`:`Semana de ${weekStart.toLocaleDateString("pt-BR")}`}</strong>
              ...
              <div className="calendar-mode-bar">
                ...
                <button className="btn ghost" onClick={goToday}>Hoje</button>
              </div>
          ...
          {view==="minha_agenda"&&
            <Card title={isMostrador?"Minha agenda de visitas":"Agenda dos mostradores"}>
...
function VisitModal({f,setF,onClose,onSave,onDelete,isAdmin,isGestor,isMostrador,isFechamento,preUsers,mostradores,editing,acoes=[],fotos=[],getUser,onCaptureLocation,onUploadFiles}){
  ...
  const statusOptions = isFechamento ? STATUS : STATUS.filter(([id])=>id!=="pos_ok");
