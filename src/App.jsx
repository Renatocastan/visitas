...
const APP_VERSION = "Castan Realtime v1.9.0-painel-admin-permissoes";
...
export default function App(){
  ...
  const canManageUsers = hasPerm("gerenciar_usuarios", isAdmin);
  const canDeleteVisits = hasPerm("excluir_visita", isAdmin);
  ...
  function canEditVisit(v){
    ...
    if(hasPerm("editar_todas_visitas", isAdmin||isGestor||isPre||isFechamento||isContratos)){
      return true;
    }

    if(hasPerm("editar_proprias_visitas", isCaptador||isMostrador)){
      return (
...
function VisitModal({f,setF,onClose,onSave,onDelete,isAdmin,isGestor,isMostrador,isFechamento,isContratos,canDeleteVisits,canStatusAvancouFechamento,canStatusPosOk,canStatusContrato,preUsers,mostradores,editing,acoes=[],fotos=[],getUser,onCaptureLocation,onUploadFiles}){
  const limited=readOnly || (isMostrador&&!isAdmin&&!isGestor);
...
function UserModal({u,setU,onClose,onSave,isAdmin}){
  ...
  return <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
    <div className="modal smallmodal">
      ...
      {isAdmin&&<div className="permissions-box">
