...
const APP_VERSION = "Castan Realtime v3.0.2-historico-acoes-captadores";
...
const visitasCanceladasBase=visitas
  ...
  function exportReport(){
    ...
    acoes.filter(a=>{
      const d=actionDateBR(a.created_at);
      return d>=reportStart && d<=reportEnd;
    }).forEach(a=>{
      const v=visitas.find(x=>x.id===a.visita_id);
      const u=getUser(a.usuario_id);
      rows.push([brDateTime(a.created_at),u?.nome||"",ROLES[u?.tipo]||u?.tipo||"",a.tipo_acao,v?.codigo_imovel||"",v?.cliente_nome||"",v?.data_visita||"",statusLabel(a.status_anterior),statusLabel(a.status_novo),brMoney(a.valor_proposta),a.observacao||""]);
    });
