...
const APP_VERSION = "Castan Realtime v3.1.4-calendario-sem-sobreposicao";
...
function Calendar({year,month,visitas,bloqueios=[],colorForUser,getUser,onNew,onEdit}){
  ...
  function minutesFromStart(hora){
    const [h,m]=String(hora||"07:00").slice(0,5).split(":").map(Number);
    return Math.max(0,((h||7)-7)*30+(m||0)/2);
  }

  function rawMinutes(hora){
  ...
  function withLanes(items){
    ...
    return placed.map(item=>({...item,total}));
  }
  ...
  return <div className="month-scheduler" onMouseLeave={()=>setTooltip(null)}>
    ...
    <div className="month-scheduler-grid">
      ...
      {Array.from({length:days}).map((_,i)=>{
        ...
        const eventos=withLanes([
          ...bloqueios.filter(b=>b.data_bloqueio===date).map(b=>({
            tipo:"bloqueio",
            id:`bloq-${b.id}`,
            item:b,
            start:rawMinutes(b.horario_inicio),
            end:rawMinutes(b.horario_fim||b.horario_inicio)+1
          })),
          ...visitas.filter(v=>v.data_visita===date).map(v=>({
            tipo:"visita",
            id:v.id,
            item:v,
            start:rawMinutes(v.horario_visita),
            end:rawMinutes(v.horario_fim_visita||v.horario_visita)+1
          }))
        ]);
        ...
        return <div className="month-day" key={day} onClick={()=>onNew(date)}>
          ...
          <div className="month-day-hours">
            ...
            {eventos.map(ev=>{
              const leftPct=(ev.lane/ev.total)*100;
              const widthPct=100/ev.total;
              const commonStyle={
                top:minutesFromStart(ev.tipo==="bloqueio"?ev.item.horario_inicio:ev.item.horario_visita),
                height:Math.max(26,minutesFromStart(ev.tipo==="bloqueio"?ev.item.horario_fim:(ev.item.horario_fim_visita||ev.item.horario_visita))-minutesFromStart(ev.tipo==="bloqueio"?ev.item.horario_inicio:ev.item.horario_visita)),
              ...
              if(ev.tipo==="bloqueio"){
                const b=ev.item;
                return <div key={ev.id} className="month-event bloqueio-event" style={commonStyle} onMouseEnter={e=>setTip(e,bloqueioTooltip(b,getUser))} onMouseMove={e=>setTip(e,bloqueioTooltip(b,getUser))} onMouseLeave={()=>setTooltip(null)} onClick={e=>e.stopPropagation()}>
              ...
              const v=ev.item;
              return <div key={ev.id} className={`month-event ${v.atualizar_fotos?"foto-destaque":""}`} style={{...commonStyle,background:colorForUser(v)}} onMouseEnter={e=>setTip(e,visitTooltip(v,getUser))} onMouseMove={e=>setTip(e,visitTooltip(v,getUser))} onMouseLeave={()=>setTooltip(null)} onClick={e=>{e.stopPropagation();onEdit(v)}}>
...
function WeeklyCalendar({weekStart,visitas,bloqueios=[],colorForUser,getUser,onNew,onEdit}){
  ...
  function minutesFromStart(hora){
    const [h,m]=String(hora||"07:00").slice(0,5).split(":").map(Number);
    return Math.max(0,((h||7)-7)*60+(m||0));
  }
  ...
  return <div className="week-scheduler" onMouseLeave={()=>setTooltip(null)}>
    ...
    <div className="week-scheduler-body"><div className="time-col">{hours.map(h=><div key={h} className="time-slot-label">{String(h).padStart(2,"0")}:00</div>)}</div>
      {days.map(d=>{
        ...
        const eventos=withLanes([
          ...bloqueios.filter(b=>b.data_bloqueio===date).map(b=>({tipo:"bloqueio",id:`bloq-${b.id}`,item:b,start:rawMinutes(b.horario_inicio),end:rawMinutes(b.horario_fim||b.horario_inicio)+1})),
          ...visitas.filter(v=>v.data_visita===date).map(v=>({tipo:"visita",id:v.id,item:v,start:rawMinutes(v.horario_visita),end:rawMinutes(v.horario_fim_visita||v.horario_visita)+1}))
        ]);

        return <div className="week-scheduler-day" key={date} onClick={()=>onNew(date)}>
          {hours.map(h=><div key={h} className="time-grid-line"></div>)}
          {eventos.map(ev=>{
            const leftPct=(ev.lane/ev.total)*100;
            const widthPct=100/ev.total;

            const startTime=ev.tipo==="bloqueio"?ev.item.horario_inicio:ev.item.horario_visita;
            const endTime=ev.tipo==="bloqueio"?ev.item.horario_fim:(ev.item.horario_fim_visita||ev.item.horario_visita);
            const commonStyle={
              top:minutesFromStart(startTime),
              height:Math.max(26,minutesFromStart(endTime)-minutesFromStart(startTime)),
              left:`calc(${leftPct}% + 6px)`,
              width:`calc(${widthPct}% - 10px)`,
              right:"auto"
            };
            ...
            if(ev.tipo==="bloqueio"){
              const b=ev.item;
              return <div key={ev.id} className="week-event bloqueio-event" style={commonStyle} onMouseEnter={e=>setTip(e,bloqueioTooltip(b,getUser))} onMouseMove={e=>setTip(e,bloqueioTooltip(b,getUser))} onMouseLeave={()=>setTooltip(null)} onClick={e=>e.stopPropagation()}>
            ...
            const v=ev.item;
            return <div key={ev.id} className={`week-event ${v.atualizar_fotos?"foto-destaque-card":""}`} style={{...commonStyle,borderLeftColor:colorForUser(v)}} onMouseEnter={e=>setTip(e,visitTooltip(v,getUser))} onMouseMove={e=>setTip(e,visitTooltip(v,getUser))} onMouseLeave={()=>setTooltip(null)} onClick={e=>{e.stopPropagation();onEdit(v)}}>
              <strong>{String(v.horario_visita).slice(0,5)}-{String(endTime).slice(0,5)} • {v.status==="reserva"?"RESERVA":v.codigo_imovel}</strong>
              <span>{v.status==="reserva"?(getUser?.(v.mostrador_id)?.nome||"Reserva"):v.cliente_nome}</span>
            </div>;
          })}
        </div>;
