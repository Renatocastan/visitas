...
const APP_VERSION = "Castan Realtime v3.1.3-calendario-sabado";
...
export default function App(){
  ...
  async function saveVisit(){
    ...
    const isReservaAgenda = f.status==="reserva";
...
async function deleteVisit(id){
  ...
  const motivoCancelamentoRows = Object.values(
    reportVisits
      ...
      .reduce((acc,v)=>{
        const motivo=v.motivo_cancelamento||"sem motivo";
        if(!acc[motivo]) acc[motivo]={motivo,total:0};
        acc[motivo].total += 1;
        return acc;
      },{})
        ...
        <main className="content">
          ...
          {view==="inicio"&&<>
            <section className="metrics">
              ...
              <Metric title="Agendadas" value={todayVisits.filter(v=>["agendada","confirmada"].includes(v.status)).length} onClick={()=>{setFilterStatus("agendada");setView("lista")}}/>
              <Metric title="Canceladas" value={visitasCanceladasBase.length} onClick={()=>setView("canceladas")}/>
              <Metric title="Remarcadas" value={todayVisits.filter(v=>v.status==="remarcada").length} onClick={()=>{setFilterStatus("remarcada");setView("lista")}}/>
            </section>
          ...
          {view==="calendario"&&<>
            ...
            <Card>
              ...
              {calendarMode==="month"
                ? <Calendar year={year} month={month} visitas={visibleVisits.filter(v=>v.status!=="cancelada")} bloqueios={agendaBloqueios.filter(b=>b.ativo!==false)} colorForUser={colorForVisit} getUser={getUser} onNew={startNewVisit} onEdit={openVisit}/>
                : calendarMode==="week"
                  ? <WeeklyCalendar weekStart={weekStart} visitas={visibleVisits.filter(v=>v.status!=="cancelada")} bloqueios={agendaBloqueios.filter(b=>b.ativo!==false)} colorForUser={colorForVisit} getUser={getUser} onNew={startNewVisit} onEdit={openVisit}/>
                  : <DailyCalendar currentDay={currentDay} visitas={visibleVisits.filter(v=>v.status!=="cancelada")} bloqueios={agendaBloqueios.filter(b=>b.ativo!==false)} colorForUser={colorForVisit} getUser={getUser} onNew={startNewVisit} onEdit={openVisit}/>
              }
            </Card>
          </>}
            ...
            <Card title="Relatórios">
              ...
              <section className="report-section">
                ...
                <div className="report-grid">
                  ...
                  <PieReport title="Motivos de cancelamento" rows={motivoCancelamentoRows} labelKey="motivo" valueKey="total"/>
                </div>
              ...
              <section className="report-section">
                <h2>Relatório de Motivos de Cancelamento</h2>
                <CancelReasonTable rows={motivoCancelamentoRows} total={canceladasRel}/>
              </section>
...
function VisitCard({v,getUser,colorForUser,onClick}){
  return <div className="visitcard" style={{borderLeftColor:colorForUser(v)}} onClick={onClick}>
    <div>
...
function Calendar({year,month,visitas,bloqueios=[],colorForUser,getUser,onNew,onEdit}){
  ...
  function minutesFromStart(hora){
    const [h,m]=String(hora||"08:00").slice(0,5).split(":").map(Number);
    return Math.max(0,((h||8)-8)*30+(m||0)/2);
  }
  ...
  return <div className="month-scheduler" onMouseLeave={()=>setTooltip(null)}>
    <div className="month-scheduler-weekdays">{WEEKDAYS.map(d=><div key={d}>{d}</div>)}</div>
    <div className="month-scheduler-grid">
      {Array.from({length:first}).map((_,i)=><div className="month-day emptyday" key={`e${i}`}/>)}
      {Array.from({length:days}).map((_,i)=>{
        const day=i+1;
        const date=`${year}-${String(month+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
        const arr=visitas.filter(v=>v.data_visita===date).sort((a,b)=>String(a.horario_visita).localeCompare(String(b.horario_visita)));
        const dayBloqueios=bloqueios.filter(b=>b.data_bloqueio===date).sort((a,b)=>String(a.horario_inicio).localeCompare(String(b.horario_inicio)));
        return <div className="month-day" key={day} onClick={()=>onNew(date)}>
          <div className="month-day-number">{day}</div>
          <div className="month-day-hours">
            {hours.map(h=><div key={h} className="month-hour-line"><span>{String(h).padStart(2,"0")}</span></div>)}
            {dayBloqueios.map(b=>{
              const top=minutesFromStart(b.horario_inicio);
              const height=Math.max(26,minutesFromStart(b.horario_fim)-top);
              return <div key={`bloq-${b.id}`} className="month-event bloqueio-event" style={{top,height}} onMouseEnter={e=>setTip(e,bloqueioTooltip(b,getUser))} onMouseMove={e=>setTip(e,bloqueioTooltip(b,getUser))} onMouseLeave={()=>setTooltip(null)} onClick={e=>e.stopPropagation()}>
                <strong>{String(b.horario_inicio).slice(0,5)} BLOQUEADO</strong>
                <span>{getUser?.(b.usuario_id)?.nome||"Mostrador"} • {b.justificativa||"Sem motivo"}</span>
              </div>
            })}
            {arr.map(v=>{
              const top=minutesFromStart(v.horario_visita);
              const fim=v.horario_fim_visita||v.horario_visita;
              const height=Math.max(26,minutesFromStart(fim)-top);
              return <div key={v.id} className={`month-event ${v.atualizar_fotos?"foto-destaque":""}`} style={{top,height,background:colorForUser(v)}} onMouseEnter={e=>setTip(e,visitTooltip(v,getUser))} onMouseMove={e=>setTip(e,visitTooltip(v,getUser))} onMouseLeave={()=>setTooltip(null)} onClick={e=>{e.stopPropagation();onEdit(v)}}>
...
function WeeklyCalendar({weekStart,visitas,bloqueios=[],colorForUser,getUser,onNew,onEdit}){
  ...
  const hours=Array.from({length:11}).map((_,i)=>8+i);
  const gridVisits=visitas.filter(v=>days.some(d=>dateStr(d)===v.data_visita)).sort((a,b)=>(String(a.data_visita)+String(a.horario_visita)).localeCompare(String(b.data_visita)+String(b.horario_visita)));
  const gridBloqueios=bloqueios.filter(b=>days.some(d=>dateStr(d)===b.data_bloqueio)).sort((a,b)=>(String(a.data_bloqueio)+String(a.horario_inicio)).localeCompare(String(b.data_bloqueio)+String(b.horario_inicio)));

  function minutesFromStart(hora){const [h,m]=String(hora||"08:00").slice(0,5).split(":").map(Number); return Math.max(0,((h||8)-8)*60+(m||0));}
  function setTip(e,text){const rect=e.currentTarget.getBoundingClientRect(); setTooltip({text,x:Math.min(rect.left,window.innerWidth-330),y:Math.max(12,rect.top-12)});}

  return <div className="week-scheduler" onMouseLeave={()=>setTooltip(null)}>
    <div className="week-scheduler-head"><div className="time-col-head"></div>{days.map(d=>{const date=dateStr(d); const isToday=date===todayISO(); return <div className={`week-scheduler-day-head ${isToday?"today":""}`} key={date}><span>{WEEKDAYS[d.getDay()]}</span><strong>{d.toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"})}</strong></div>})}</div>
    <div className="week-scheduler-body"><div className="time-col">{hours.map(h=><div key={h} className="time-slot-label">{String(h).padStart(2,"0")}:00</div>)}</div>
      {days.map(d=>{const date=dateStr(d); const arr=gridVisits.filter(v=>v.data_visita===date); const dayBloqueios=gridBloqueios.filter(b=>b.data_bloqueio===date); return <div className="week-scheduler-day" key={date} onClick={()=>onNew(date)}>
        {hours.map(h=><div key={h} className="time-grid-line"></div>)}
        {dayBloqueios.map(b=>{const top=minutesFromStart(b.horario_inicio); const height=Math.max(26,minutesFromStart(b.horario_fim)-top); return <div key={`bloq-${b.id}`} className="week-event bloqueio-event" style={{top,height}} onMouseEnter={e=>setTip(e,bloqueioTooltip(b,getUser))} onMouseMove={e=>setTip(e,bloqueioTooltip(b,getUser))} onMouseLeave={()=>setTooltip(null)} onClick={e=>e.stopPropagation()}><strong>{String(b.horario_inicio).slice(0,5)}-{String(b.horario_fim).slice(0,5)} • BLOQUEADO</strong><span>{getUser?.(b.usuario_id)?.nome||"Mostrador"} • {b.justificativa||"Sem motivo"}</span></div>})}
        {arr.map(v=>{const top=minutesFromStart(v.horario_visita); const fim=v.horario_fim_visita||v.horario_visita; const height=Math.max(26,minutesFromStart(fim)-top); return <div key={v.id} className={`week-event ${v.atualizar_fotos?"foto-destaque-card":""}`} style={{top,height,borderLeftColor:colorForUser(v)}} onMouseEnter={e=>setTip(e,visitTooltip(v,getUser))} onMouseMove={e=>setTip(e,visitTooltip(v,getUser))} onMouseLeave={()=>setTooltip(null)} onClick={e=>{e.stopPropagation();onEdit(v)}}><strong>{String(v.horario_visita).slice(0,5)}-{String(fim).slice(0,5)} • {v.status==="reserva"?"RESERVA":v.codigo_imovel}</strong><span>{v.status==="reserva"?(getUser?.(v.mostrador_id)?.nome||"Reserva"):v.cliente_nome}</span></div>})}
      </div>})}
    </div>
    {tooltip&&<div className="calendar-tooltip-floating" style={{left:tooltip.x,top:tooltip.y}}>{tooltip.text}</div>}
  </div>;
...
function CancelReasonTable({rows,total}){
  return <div className="tablewrap">
    <table>
      <thead><tr><th>Motivo</th><th>Quantidade</th><th>Percentual</th></tr></thead>
      <tbody>{rows.map(r=><tr key={r.motivo}><td>{r.motivo}</td><td>{r.total}</td><td>{r.percentual}</td></tr>)}</tbody>
