...
const APP_VERSION = "Castan Realtime v3.1.9-ajustes-reserva-cancelada";
...
async function deleteVisit(id){
  ...
  const motivoCancelamentoRows = Object.values(
    reportVisits
      ...
      .reduce((acc,v)=>{
        const motivo = v.motivo_cancelamento==="outros"
          ? `outros: ${String(v.motivo_cancelamento_outros||"sem descrição").trim()}`
          : (v.motivo_cancelamento||"sem motivo");
        ...
        <main className="content">
          ...
          {view==="calendario"&&<>
            ...
            <Card>
              ...
              {calendarMode==="month"
                ? <Calendar year={year} month={month} visitas={visibleVisits.filter(v=>!["cancelada","reserva_cancelada"].includes(v.status))} bloqueios={agendaBloqueios.filter(b=>b.ativo!==false)} colorForUser={colorForVisit} getUser={getUser} onNew={startNewVisit} onEdit={openVisit}/>
                : calendarMode==="week"
                  ? <WeeklyCalendar weekStart={weekStart} visitas={visibleVisits.filter(v=>!["cancelada","reserva_cancelada"].includes(v.status))} bloqueios={agendaBloqueios.filter(b=>b.ativo!==false)} colorForUser={colorForVisit} getUser={getUser} onNew={startNewVisit} onEdit={openVisit}/>
                  : <DailyCalendar currentDay={currentDay} visitas={visibleVisits.filter(v=>!["cancelada","reserva_cancelada"].includes(v.status))} bloqueios={agendaBloqueios.filter(b=>b.ativo!==false)} colorForUser={colorForVisit} getUser={getUser} onNew={startNewVisit} onEdit={openVisit}/>
              }
            </Card>
          </>}
