...
const APP_VERSION = "Castan Realtime v3.0.6-calendario-tooltip-hora";
    ...
    <div className="calendar">
      ...
      {Array.from({length:days}).map((_,i)=>{
        ...
        return <div className="day" key={day} onClick={()=>onNew(date)}>
          <b>{day}</b>
          {arr.map(v=>
            <div className={`chip ${v.atualizar_fotos?"foto-destaque":""}`} data-tooltip={visitTooltip(v,getUser)} key={v.id} style={{background:colorForUser(v.mostrador_id)}} onClick={e=>{e.stopPropagation();onEdit(v)}}>
    ...
    {days.map(d=>{
      ...
      return <div className={`weekly-day ${isToday?"today":""}`} key={date} onClick={()=>onNew(date)}>
        ...
        {arr.length?arr.map(v=>
          <div className={`weekly-visit ${v.atualizar_fotos?"foto-destaque-card":""}`} data-tooltip={visitTooltip(v,getUser)} key={v.id} style={{borderLeftColor:colorForUser(v.mostrador_id)}} onClick={e=>{e.stopPropagation();onEdit(v)}}>
            <strong>{String(v.horario_visita).slice(0,5)} • {v.codigo_imovel}</strong>
            <span>{v.cliente_nome}</span>
          </div>
...
function DailyCalendar({currentDay,visitas,colorForUser,getUser,onNew,onEdit}){
  ...
  return <div className="daily-calendar">
    <div className="daily-day" onClick={()=>onNew(currentDay)}>
      ...
      {arr.length?arr.map(v=>
        <div className={`weekly-visit daily-visit ${v.atualizar_fotos?"foto-destaque-card":""}`} data-tooltip={visitTooltip(v,getUser)} key={v.id} style={{borderLeftColor:colorForUser(v.mostrador_id)}} onClick={e=>{e.stopPropagation();onEdit(v)}}>
          <strong>{String(v.horario_visita).slice(0,5)} • {v.codigo_imovel}</strong>
          <span>{v.cliente_nome}</span>
