...
const APP_VERSION = "Castan Realtime v2.0.0-ajustes-agenda";
        ...
        <main className="content">
          ...
          {view==="canceladas"&&
            <Card title="Visitas canceladas">
              {visitas.filter(v=>v.status==="cancelada").sort((a,b)=>(b.data_visita+String(b.horario_visita)).localeCompare(a.data_visita+String(a.horario_visita))).map(v=>
                <VisitCard key={v.id} v={v} getUser={getUser} colorForUser={colorForUser} onClick={()=>openVisit(v)}/>
              )}
              {!visitas.filter(v=>v.status==="cancelada").length&&<Empty text="Nenhuma visita cancelada."/>}
            </Card>
          }
